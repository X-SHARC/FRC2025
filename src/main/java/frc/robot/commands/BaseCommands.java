...
public class BaseCommands {
    ...
    public static Command homeElevator(Elevator elevator, Outtake outtake) {
        double meters = Constants.FieldConstants.heightMap.get(Height.L1).doubleValue();
        return new SequentialCommandGroup(setPivot(outtake, 5), setElevator(elevator, meters),
                new InstantCommand(() -> elevator.stop(), elevator));
    }

    public static Command manipulateObject(Outtake outtake, double volts, double angle, BooleanSupplier isFinished) {
        return new FunctionalCommand(
                () -> {
                },
                () -> {
